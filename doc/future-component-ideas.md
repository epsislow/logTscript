...
**Done:** [debug.md](debug.md) — `watch()` section; panel in `index.html` (**Panels → Timeline**). Supports multi-bit wire expansion (`watch(o)` → `o.0`…), bit ranges (`watch(o.1-3)`), and component properties (`watch(.o:counter)`). Editor-only; not in `run_tests.html`.
