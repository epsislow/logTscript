...
const SCRIPT_EDITOR_HTML = path.join(ROOT, 'index.html');
...
const GROUP_META = [
  ...
  { id: 'parser', label: 'Inline parser (inline [parser])' },
...
function syncScriptEditorHtml() {
  ...
  if (!re.test(html)) {
    throw new Error('index.html: missing ' + EDITOR_PIPELINE_START);
  }
  html = html.replace(re, block);
...
function validateScriptEditorHtml() {
  ...
  if (missing.length) {
    throw new Error(
      'index.html missing script tags (add manually):\n  ' + missing.join('\n  ')
    );
  }
}
